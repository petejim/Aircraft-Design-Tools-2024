function [W_Sval,W_Pval,P_Wval,minhp,hp] ... 
    = constDiagramFunction1(plotConstraint,plot2scale,cruisealt,takeoffalt,cruisev,...
    runway_length,dh_dt,L_D,Clmax,CD0,eta,horsepower,W_toval,inputVariables)
    ...
    W_Sminval = W_S(P_Wcruisetakeoffindex1);
    P_Wminval = P_Wcruise(P_Wcruisetakeoffindex1);
    
    % Get Input W_S
    W_Sval = inputVariables(12);
    for i = 1:length(W_S)
        if W_S(i) >= W_Sval
            W_Svalindex = i;
            break
    ...
    if W_Sval > W_Sminval
        P_Wval = P_Wtakeoff(W_Svalindex);
    else
        P_Wval = P_Wcruise(W_Svalindex); 
    end
    ...
    %         W_Sminval = W_S(P_Whptakeoffindex1);
    %         P_Wminval = P_Wtakeoff(P_Whptakeoffindex1);
    %     
    %         % Find Intersection with Hp and Landing Line
    %         if W_Shptakeoff >= W_Sland
    %             W_Sminval = W_Sland;
    %             P_Wminval = P_Whorsepower;
    %         end
    %     
    %         plot(W_Sminval,P_Wminval,'o','MarkerSize', 10,'MarkerFaceColor','k');
    %     end
        ...
        ylabel('Power to Weight: P_W (hp/lbs)', 'FontSize', 14, 'FontName', 'Calibri');
        xlabel('Wing Loading: W_S (lbs/ft^2)', 'FontSize', 14, 'FontName', 'Calibri');
        set(gcf,'color','w');
        grid on;
