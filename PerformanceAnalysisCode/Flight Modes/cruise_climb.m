...
function [AS] = cruise_climb(sectors, sector_num, AP, AS, engine_info, all_tailwind, distance)
            % = cruise_cnst_CL_v2(sector, sector_num, AP, AS, engine_info, all_tailwind,distance,,,CL,CD);
...
% Mission Inputs
dt_sec = sectors(sector_num,7);
v_cruise_knots = sectors(sector_num,4);
distance_target_nm = sectors(sector_num,6);

% Engine Inputs
EngineType = engine_info.engine_type;
SeaLevelEngine = engine_info.sea_level_engine;
MinSFC = engine_info.min_SFC;
service_ceiling = engine_info.service_ceiling;
n = engine_info.resolution;

% CL and CD
[CL, CD] = cruise_climb_CL_CD_finder(sectors, sector_num, AP, AS);
